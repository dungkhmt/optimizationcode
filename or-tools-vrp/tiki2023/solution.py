...
def validate(solution: Solution, problem: Problem) -> str:
...
def analyze_routes(solution: Solution, problem: Problem):
    header = ['Request ID', 'Action', 'Hub', 'Weight', 'Request Volume', 'Begin Serve Time', 'Arrival Time', 'End Serve Time',
              'Load', 'Truck Volume']

    for route_id, route in enumerate(solution):
        print(f'Detail of route {route_id}:\n')
        ...
        for request_id in route:
            ...
            try:
                ...
                if is_pickup:
                    data.append([request_id, 'pickup failed', request.pickup_hub, request.weight, request.volume, request.start_pickup_time,
                                 truck_state.arrival_time, request.end_pickup_time, '-', '-'])
                else:
                    data.append([request_id, 'drop failed', request.drop_hub, request.weight, request.volume, request.start_drop_time,
            ...
            if is_pickup:
                data.append([request_id, 'pickup', request.pickup_hub, request.weight, request.volume, request.start_pickup_time,
                             truck_state.arrival_time, request.end_pickup_time, truck_state.load, truck_state.volume])
            else:
                data.append([request_id, 'drop', request.drop_hub, request.weight, request.volume, request.start_drop_time,
...
def analyze_solution_quality(solution: Solution, problem: Problem):
    request_served = int(sum(len(route) for route in solution) / 2)
    truck_used = sum(1 if route else 0 for route in solution)

    print(f'Solution status: {validate(solution, problem)}')
    print(f'Total request served: {request_served}/{problem.request_count}')
    print(f'Total truck used: {truck_used}/{problem.truck_count}')


if __name__ == '__main__':
    prb = Problem('datasets/sample.txt')
    sol = [[7, 9, 4, 5, 6, 0, 5, 9, 4, 7, 3, 1, 6, 0, 8, 1, 2, 3, 2, 8]]
