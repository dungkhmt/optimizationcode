...
constraints = []
f0 = [PlusVarConst(x[i],0,'f0[' + str(i) + ']') for i in range(n)] # f0[i] = x[i]
f1 = [PlusVarConst(x[i],i,'f1[' + str(i) + ']') for i in range(n)] # f1[i] = x[i] + i 
f2 = [PlusVarConst(x[i],-i,'f2[' + str(i) + ']') for i in range(n)] # f2[i] = x[i] - i
